...
describe('CarouselComponent', () => {
  ...
  it('should render carousel with slides keeping default values', waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(carouselHTML.classList.contains('owl-loaded')).toBeTruthy(
        'has .owl-loaded'
      );
      ...
      expect(carouselHTML.classList.contains('owl-grab')).toBeFalsy(
        'has .owl-grab'
      );

      expect(carouselComponent.carouselLoaded).toBeTruthy(
        'owlVisible should be true; this means stage is created'
      );
      ...
      expect(deStage.nativeElement.style.transitionDuration).toBe(
        '0s',
        'transition of stage'
      );
      ...
      expect(deStage.nativeElement.style.transform).toBe(
        'translate3d(0px, 0px, 0px)',
        'transform of stage'
      );
      ...
      expect(deStage.nativeElement.style.paddingRight).toBe(
        '',
        'padding-right of stage'
      );
      ...
      expect(deSlides[0].nativeElement.classList.contains('active')).toBeTruthy(
        'startPosition 0, first slide must be active'
      );

      expect(deSlides[0].nativeElement.clientWidth).toBe(
        400,
        '400px width of first slide'
      );
      expect(deSlides[0].nativeElement.style.marginLeft).toBe(
        '',
        '"" margin-left of first slide'
      );
      expect(deSlides[0].nativeElement.style.marginRight).toBe(
        '',
        '"" margin-left of first slide'
      );
      expect(deSlides[0].nativeElement.style.marginRight).toBe(
      ...
      const clonedSlides: DebugElement[] = deCarouselComponent.queryAll(
        By.css('.owl-item.cloned')
      );
      expect(clonedSlides.length).toBe(0, '0 cloned slides');

      const centeredSlides: DebugElement[] = deCarouselComponent.queryAll(
        By.css('.owl-item.center')
      );
  ...
  it(`should render carousel with 5 active slides when prop items=10 and there're just 5 slides: [options]="{items: '10'}"`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(deDots[0].nativeElement.classList.contains('disabled')).toBeTruthy(
        'has .disabled class'
      );
    });
  }));
  ...
  and each slide has width of 10th part of <owl-carousel-o> [options]="{items: '10', skip_validateItems: true}"`, fakeAsync(() => {
    ...
    expect(stageWidth).toEqual(
      slideWidth * 5,
      'slide has width which is 5th part of .owl-stage'
    );
  ...
  it(`should render carousel with 2 active slides and 6 cloned slide (total slides 11) [options]="{items: 2, loop: true}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      const clonedUnactiveSlides: DebugElement[] = deCarouselComponent.queryAll(
        By.css('.owl-item.cloned.active')
      );
      expect(clonedUnactiveSlides.length).toBe(
        0,
        "when startPosition is 0 cloned slides shouldn't have .active"
      );
    });
  }));
  ...
  it(`should render carousel with 5th active slide and 1 cloned active slide (total slides 11)
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(activeSlides.length).toBe(2, '2 active slides');
      expect(activeSlides[0].nativeElement.innerHTML).toContain(
        'Slide 5',
        'first active slide is last slide of origin item'
      );

      expect(
        activeSlides[1].nativeElement.classList.contains('cloned')
      ).toBeTruthy('second active element is cloned');
      expect(activeSlides[1].nativeElement.innerHTML).toContain(
        'Slide 1',
        'second active slide is clone of first origin item'
      );
    });
  }));
  ...
  it(`should render carousel with 2th active slide [options]="{items: 2, startPosition: 1}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(activeSlides[0].nativeElement.innerHTML).toContain(
        'Slide 2',
        'first active slide is second slide of all slides'
      );

      deSlides = deCarouselComponent.queryAll(By.css('.owl-item'));
      expect(deSlides[0].nativeElement.classList.contains('active')).toBeFalsy(
        "first slide doesn't have class .active"
      );
    });
  }));
  ...
  it(`should render carousel with 4th active slide not 5th[options]="{items: 2, startPosition: 4}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(activeSlides[0].nativeElement.innerHTML).toContain(
        'Slide 4',
        'first active slide is 4th slide of all slides'
      );
      expect(activeSlides[1].nativeElement.innerHTML).toContain(
        'Slide 5',
        'second active slide is 5th slide of all slides'
      );
    });
  }));
  ...
  it(`should render carousel which has 1th slide with class .center [options]="{items: 2, center: true}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      const activeCenterSlide: DebugElement[] = deCarouselComponent.queryAll(
        By.css('.owl-item.active.center')
      );
      expect(activeCenterSlide.length).toBe(1, '1 centered slide');
      expect(activeCenterSlide[0].nativeElement.innerHTML).toContain(
        'Slide 1',
        '1th slide of all slides has class .center'
      );
    });
  }));
  ...
  it(`should render carousel which has 3th slide with class .center [options]="{items: 2, center: true, startPosition: 2}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(activeCenterSlide[0].nativeElement.innerHTML).toContain(
        'Slide 3',
        '3th slide of all slides has class .center'
      );
      const activeSlides: DebugElement[] = deCarouselComponent.queryAll(
        By.css('.owl-item.active')
      );
      expect(activeSlides.length).toBe(
        3,
        "however items=2 .active will get 3 slides. That's because Slide 3 will take center and sibling slides will be visible partly"
      );
    });
  }));
  ...
  it('should stop centering the slides when the width of the carousel is between 600 and 900', fakeAsync(() => {
    ...
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeTruthy('1th active slide is cloned');
    expect(
      deActiveSlides[1].nativeElement.classList.contains('center')
    ).toBeTruthy("2th slide has css-class '.center'");
    ...
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeFalsy("1th active slide isn't cloned");
    const centeredSlide: DebugElement = deCarouselComponent.query(
      By.css('.owl-item.center')
    );
  ...
  it(`should render carousel which doesn\'t have class .owl-drag [options]="{items: 1, mouseDrag: false}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(carouselHTML.classList.contains('owl-drag')).toBeFalsy(
        "shouldn't have class .owl-drag"
      );
    });
  }));
  ...
  it(`should render carousel with slides having 'margin-right=10px' [options]="{items: 2, margin: 10}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(deSlides[0].nativeElement.clientWidth).toBe(
        595,
        'width of each slider should be 595px'
      );
    });
  }));
  ...
  it(`should render carousel with class .owl-rtl, active slides starting from 2nd position and first of active slide should have .center  [options]="{items: 3, startPosition: 2, rtl: true}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(activeSlides[0].nativeElement.innerHTML).toContain(
        'Slide 2',
        'Slide 3 should be first, but due to center=true and odd number of active slides, it takes center spot giving its place preceding Slide 2'
      );
      expect(
        activeSlides[1].nativeElement.classList.contains('center')
      ).toBeTruthy('Slide 3 is centered');
      expect(activeSlides.length).toBe(3, '3 active slides');
    });
  }));
  ...
  it(`should render responsive carousel  [options]="{nav: true, responsive: {'0': {items: 1}, '600': {items: 2}, '900': {items: 3}}}"`, fakeAsync(() => {
    ...
    expect(carouselHTML.classList.contains('owl-responsive')).toBeTruthy(
      'should have class .owl-responsive'
    );

    let activeSlides: DebugElement[] = deCarouselComponent.queryAll(
      By.css('.owl-item.active')
    );
    ...
    ).toBeTruthy('prev button is disabled');
  ...
  it(`should change settings according to the responsive object in different viewsports `, fakeAsync(() => {
    ...
    expect(activeSlides.length).toBe(3, 'should be 3 active slides');
    expect(activeSlides[0].nativeElement.style.marginRight).toBe(
      '',
      `margin-right: ''`
    );
  ...
  it(`should set paddings to .owl-stage  [options]="{items: 3, loop: true, stagePadding: 40}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      fixtureHost.detectChanges();
      expect(getComputedStyle(deStage.nativeElement).paddingLeft).toBe(
        '40px',
        'padding-left should be 40px'
      );
      expect(getComputedStyle(deStage.nativeElement).paddingRight).toBe(
        '40px',
        'padding-right should be 40px'
      );
    });
  }));
  ...
  describe(`THE OPTION 'MERGE'`, () => {
    it(`should render carousel with option merge (in fact this option does almost nothing; we should use data-binding to directive carouselSlide: [dataMerge]="2") [options]="{merge: true}`, waitForAsync(() => {
      ...
      fixtureHost.whenStable().then(() => {
        ...
        expect(deSlides[0].nativeElement.clientWidth).toBe(
          800,
          'width of 1th slide is twice bigger then common slide'
        );
        expect(deSlides[1].nativeElement.clientWidth).toBe(
          400,
          'width of 2th slide should be 400 (1200/3=400)'
        );
        expect(deSlides[2].nativeElement.clientWidth).toBe(
          800,
          'width of 3th slide is twice bigger then common slide'
        );
      });
    }));
    ...
    it(`should cancel the influence of 'merge' when the width of the carousel is between 600 and 900`, fakeAsync(() => {
      const html = `
        <div style="width: 1200px; margin: auto">
          <div class="owl-wrapper">
            <owl-carousel-o [options]="{ merge: true,
      ...
      expect(deSlides[0].nativeElement.clientWidth).toBe(
        268,
        'width of 1th slide should be 268 (804/3=268)'
      );
      expect(deSlides[1].nativeElement.clientWidth).toBe(
        268,
        'width of 2th slide should be 268 (804/3=268)'
      );
      expect(deSlides[2].nativeElement.clientWidth).toBe(
        268,
        'width of 3th slide should be 268 (804/3=268)'
      );
      ...
      expect(deSlides[1].nativeElement.clientWidth).toBe(
        134,
        'width of 2th slide should be 400 (402/3=134)'
      );
      expect(deSlides[2].nativeElement.clientWidth).toBe(
        268,
        'width of 3th slide is twice bigger then common slide'
    ...
    it(`should cancel the influence of 'mergeFit' when the width of the carousel is between 600 and 900`, fakeAsync(() => {
      const html = `
        <div style="width: 1200px; margin: auto">
          <div class="owl-wrapper">
            <owl-carousel-o [options]="{ merge: true,
      ...
      expect(deSlides[0].nativeElement.clientWidth).toBe(
        1072,
        'width of 1th slide should be 1072 (804/3*4=1072)'
      );
      expect(deSlides[1].nativeElement.clientWidth).toBe(
        268,
        'width of 2th slide should be 268 (804/3=268)'
      ...
      expect(deSlides[2].nativeElement.clientWidth).toBe(
        536,
        'width of 3th slide should be 536 (804/3*2=536)'
      );
  ...
  describe(`THE OPTION 'AUTOWIDTH'`, () => {
    ...
    it(`should cancel the 'autoWidth' when the width of the carousel is between 600 and 900`, fakeAsync(() => {
      ...
      expect(deSlides[1].nativeElement.clientWidth).toBe(
        134,
        'width of 2th slide should be 134 (402/3=134)'
      );
      expect(deSlides[2].nativeElement.clientWidth).toBe(
        500,
        'width of 3th slide is 500px'
  ...
  it(`should add navigation buttons  [options]="{nav: true}`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(prevButton.innerHTML).toContain(
        'prev',
        'prev is text of prev button'
      );
      expect(nextButton.innerHTML).toContain(
        'next',
        'next is text of next button'
      );
    });
  }));
  ...
  it(`should render carousel with dots with text [options]="{dotsData: true}"`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      deDots = deCarouselComponent.queryAll(By.css('.owl-dots > .owl-dot'));
      expect(deDots.length).toBe(5, '5 dots');
      expect(deDots[0].nativeElement.innerHTML).toContain(
        'to Slide 1',
        'to Slide 1'
      );
      expect(
        deDots[0].nativeElement.classList.contains('owl-dot-text')
      ).toBeTruthy('dot has .owl-dot-text');
      deDots = deCarouselComponent.queryAll(
        By.css('.owl-dots > .owl-dot.owl-dot-text')
      );
      expect(deDots.length).toBe(5, '5 dots');
    });
  }));
  ...
  it(`should render carousel with dots with html in [options]="{dotsData: true}"`, waitForAsync(() => {
    ...
    fixtureHost.whenStable().then(() => {
      ...
      expect(deDots.length).toBe(5, '5 dots');
      const innerSpan =
        deDots[0].nativeElement.firstElementChild.firstElementChild;
      expect(innerSpan.classList.contains('innerDot')).toBeTruthy(
        'dot contains <span class=innerDot>1<span>'
      );
    });
  }));
  ...
  it(`should render 3 dots with [options]="{loop: true, merge: true}" and defined [dataMerge]; ; and move carousel correctly by clicking dots`, fakeAsync(() => {
    ...
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeTruthy('Slide 4 is cloned');
    expect(deActiveSlides[1].nativeElement.innerHTML).toContain(
      'Slide 5',
      'Slide 5'
    );
    expect(
      deActiveSlides[1].nativeElement.classList.contains('cloned')
    ).toBeTruthy('Slide 5 is cloned');
  ...
  it(`should render 5 dots with [options]="{autoWidth: true, loop: true}" and defined [width]; and move carousel by clicking on dots`, fakeAsync(() => {
    ...
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeFalsy("Slide 1 isn't cloned");
    ...
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeFalsy("Slide 3 isn't cloned");
    ...
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeFalsy("Slide 5 isn't cloned");
    expect(deActiveSlides[1].nativeElement.innerHTML).toContain(
      'Slide 1',
      'Slide 1'
    );
  ...
  describe(`THE OPTION 'REWIND'`, () => {
    ...
    it('should stop rewinding when the width of the carousel is between 600 and 900', fakeAsync(() => {
      ...
      expect(prevButton.classList.contains('disabled')).toBeFalsy(
        "prev nav button hasn't class .disabled"
      );
      ...
      expect(prevButton.classList.contains('disabled')).toBeTruthy(
        'prev nav button has class .disabled'
      );
      deNavButtons[0].triggerEventHandler('click', null);
      tick();
      fixtureHost.detectChanges();
  ...
  it('should loop stage movement by using nav prev-next buttons [options]="{nav: true, loop: true}"', fakeAsync(() => {
    ...
    tick(31);
    fixtureHost.detectChanges();
    expect(deSlides[2].nativeElement.classList.contains('active')).toBeTruthy(
      '3th slide is active'
    );
    expect(deSlides[2].nativeElement.classList.contains('cloned')).toBeTruthy(
      '3th slide is active and cloned'
    );
    expect(deDots[1].nativeElement.classList.contains('active')).toBeTruthy(
      '2th dot is active'
    );
    ...
    tick(31);
    fixtureHost.detectChanges();
    expect(deSlides[6].nativeElement.classList.contains('active')).toBeTruthy(
      '6th slide at all and 4th from origin slides is active'
    );
    expect(deSlides[6].nativeElement.innerHTML).toContain('Slide 4');
    expect(deSlides[8].nativeElement.classList.contains('active')).toBeTruthy(
      '8th slide which is clone of 1th origin slide is active'
    );
    ...
    expect(deActiveSlides[0].nativeElement.innerHTML).toContain(
      'Slide 5',
      '5th origin slide is active'
    );
    expect(
      deActiveSlides[0].nativeElement.classList.contains('cloned')
    ).toBeFalsy('1th active slide is origin and not cloned');
    expect(
      deActiveSlides[1].nativeElement.classList.contains('cloned')
    ).toBeTruthy('2th active slide is cloned');
    expect(deActiveSlides[1].nativeElement.innerHTML).toContain(
      'Slide 1',
      '2th active slide is the clone of 1th origin slide'
    );
    expect(
      deActiveSlides[2].nativeElement.classList.contains('cloned')
    ).toBeTruthy('3th active slide is cloned');
    expect(deActiveSlides[2].nativeElement.innerHTML).toContain(
      'Slide 2',
      '3th active slide is the clone of 2th origin slide'
    );
    expect(deDots[1].nativeElement.classList.contains('active')).toBeTruthy(
      '2th dot is active'
    );
    ...
    expect(deActiveSlides[1].nativeElement.innerHTML).toContain(
      'Slide 2',
      '2th origin slide is active'
    );
    expect(
      deActiveSlides[1].nativeElement.classList.contains('cloned')
    ).toBeFalsy('2th active slide is origin and not cloned');
    expect(deActiveSlides[2].nativeElement.innerHTML).toContain(
      'Slide 3',
      '3th origin slide is active'
    );
    expect(
      deActiveSlides[2].nativeElement.classList.contains('cloned')
    ).toBeFalsy('3th active slide is origin and not cloned');
    expect(deDots[0].nativeElement.classList.contains('active')).toBeTruthy(
      '2th dot is active'
    );
  ...
  it(`should change the centered slide after clicking prev and next buttons; [options]="{nav: true, center: true, loop: true}"`, fakeAsync(() => {
    ...
    expect(deDots[4].nativeElement.classList.contains('active')).toBeTruthy(
      'last dot has .active'
    );
  ...
  describe(`THE OPTION 'SLIDEBY'`, () => {
    ...
    it(`should move carousel by pages; [options]="{nav: true, slideBy: 'page'}"`, fakeAsync(() => {
      ...
      expect(
        deNavButtons[0].nativeElement.classList.contains('disabled')
      ).toBeFalsy('prev button is enabled');
    ...
    it(`should move carousel by pages; [options]="{nav: true, slideBy: 'page', rewind: true}"`, fakeAsync(() => {
      ...
      expect(
        ...
        'prev button is enabled; click on it rewinds the carousel to the end'
      );
      expect(deDots.length).toBe(4, '4 dots = 4 pages');
      expect(deDots[0].nativeElement.classList.contains('active')).toBeTruthy(
        '1th dot is active'
      ...
      expect(
        ...
        'next button is enabled; click on it rewinds the carousel to the begin without loop effect'
      );
      ...
      expect(
        ...
        'prev button is disabled; click on it rewinds the carousel to the end'
      );
      ...
      expect(
        ...
        'next button is disabled; click on it rewinds the carousel to the begin'
      );
    }));

    it(`should move the carousel by pages; [options]="{nav: true, slideBy: 'page', loop: true}"`, fakeAsync(() => {
      const html = `
        <div style="width: 920px; margin: auto">
      ...
      expect(
        deActiveSlides[1].nativeElement.classList.contains('cloned')
      ).toBeTruthy('Slide 1 which is after Slide 10 is cloned');
      expect(deActiveSlides[2].nativeElement.innerHTML).toContain(
        'Slide 2',
        'Slide 2'
      );
      expect(
        deActiveSlides[2].nativeElement.classList.contains('cloned')
      ).toBeTruthy('Slide 2 which is after Slide 10 is cloned');
      expect(deDots[3].nativeElement.classList.contains('active')).toBeTruthy(
        '4th dot is active'
      );
      ...
      const deCloned: DebugElement[] = deCarouselComponent.queryAll(
        By.css('.owl-item.active.cloned')
      );
      ...
      expect(
        deActiveSlides[0].nativeElement.classList.contains('cloned')
      ).toBeTruthy('Slide 10 which is before Slide 1 is cloned');
      expect(deActiveSlides[1].nativeElement.innerHTML).toContain(
        'Slide 1',
        'Slide 1'
      );
  ...
  it('should move carousel onto 1 slide [options]="{nav: true, dotsEach: true, loop: true}"', fakeAsync(() => {
    ...
    expect(
      deActiveSlides[1].nativeElement.classList.contains('cloned')
    ).toBeFalsy("Slide 2 isn't cloned");
    expect(deActiveSlides[2].nativeElement.innerHTML).toContain(
      'Slide 2',
      'Slide 2'
    );
    ...
    const deActiveClonedItems: DebugElement[] = deCarouselComponent.queryAll(
      By.css('.owl-item.active.cloned')
    );
  ...
  it('should pass data after moving carousel [options]="{nav: true}"', fakeAsync(() => {
    ...
    const stageComponent: DebugElement = fixtureHost.debugElement.query(
      By.css('owl-stage')
    );
    tick();
  ...
  it('should pass data after event \'changed\' fires [options]="{nav: true}"', fakeAsync(() => {
    discardPeriodicTasks();
    const html = `
      <div style="width: 1200px; margin: auto">
        <owl-carousel-o [options]="{nav: true}" (changed)="getCurrentlyPassedData($event)" (translated)="getPassedData($event)">
          <ng-template carouselSlide id="owl-slide-1">Slide 1</ng-template>
          <ng-template carouselSlide id="owl-slide-2">Slide 2</ng-template>
          <ng-template carouselSlide id="owl-slide-3">Slide 3</ng-template>
          <ng-template carouselSlide id="owl-slide-4">Slide 4</ng-template>
  ...
  describe(`THE OPTION 'AUTOPLAY`, () => {
    ...
    it("should stop autoplaying carousel by 'mouseover' and 'touchstart'[options]=\"{nav: true, loop: true, autoplay: true, autoplayTimeout: 100, autoplayHoverPause: true}\"", fakeAsync(() => {
      ...
      autoplayService = deCarouselComponent.injector.get(AutoplayService);
      spyOn(autoplayService, 'startPausing').and.callThrough();
      spyOn(autoplayService, 'startPlayingMouseLeave').and.callThrough();
      spyOn(autoplayService, 'startPlayingTouchEnd').and.callThrough();
    ...
    it("should start autoplaying carousel after 80 ms when 'mouseover' and 'mouseleave' [options]=\"{nav: true, loop: true, autoplay: true, autoplayTimeout: 100, autoplayHoverPause: true, autoplayMouseleaveTimeout: 80}\"", fakeAsync(() => {
      discardPeriodicTasks();
      const html = `
        <div style="width: 1200px; margin: auto">
          <owl-carousel-o [options]="{
      ...
      // can't imitate passage of time (tick(130) doesn't wait). Thus next checking won't work
      // deActiveSlides = deCarouselComponent.queryAll(By.css('.owl-item.active'));
  ...
  describe(`THE OPTION 'slideTransition'`, () => {
    it(`transition property of '.owl-stage' should be 'ease' if 'slideTransition' isn't set up`, fakeAsync(() => {
      ...
      deStage = deCarouselComponent.query(By.css('.owl-stage'));
      expect(
        getComputedStyle(deStage.nativeElement).transitionTimingFunction
      ).toBe('ease', 'ease');
    }));
  ...
  it("should animate slides [options]=\"{nav: true, items: 1, animateOut: 'slideOutDown', animateIn: 'flipInX'}\"", fakeAsync(() => {
    ...
    expect(oldActiveSlide.classList.contains('animated')).toBeTruthy(
      '1th slide has .animated class'
    );
    expect(oldActiveSlide.classList.contains('owl-animated-out')).toBeTruthy(
      '1th slide has .owl-animated-out class'
    );
    expect(oldActiveSlide.classList.contains('slideOutDown')).toBeTruthy(
      '1th slide has .slideOutDown class'
    );
    ...
    expect(newActiveSlide.classList.contains('animated')).toBeTruthy(
      '2th slide has .animated class'
    );
    expect(newActiveSlide.classList.contains('owl-animated-in')).toBeTruthy(
      '2th slide has .owl-animated-in class'
    );
    expect(newActiveSlide.classList.contains('flipInX')).toBeTruthy(
      '2th slide has .flipInX class'
    );
    expect(newActiveSlide.classList.contains('active')).toBeTruthy(
      '2th slide has .active class'
    );
    ...
    expect(deSlides[0].nativeElement.classList.contains('animated')).toBeFalsy(
      "1th slide hasn't .animated class"
    );
    expect(oldActiveSlide.classList.contains('owl-animated-out')).toBeFalsy(
      "1th slide hasn't .owl-animated-out class"
    );
    ...
    expect(newActiveSlide.classList.contains('animated')).toBeFalsy(
      "2th slide hasn't .animated class"
    );
    expect(newActiveSlide.classList.contains('owl-animated-in')).toBeFalsy(
      "2th slide hasn't .owl-animated-in class"
    );
    expect(newActiveSlide.classList.contains('flipInX')).toBeFalsy(
      "2th slide hasn't .flipInX class"
    );
  }));
  ...
  it("should animate just new active slides; 'animateOut' is absent [options]=\"{nav: true, items: 1, animateIn: 'flipInX'}\"", fakeAsync(() => {
    ...
    expect(oldActiveSlide.classList.contains('animated')).toBeFalsy(
      "1th slide hasn't .animated class"
    );
    expect(oldActiveSlide.classList.contains('owl-animated-out')).toBeFalsy(
      "1th slide hasn't .owl-animated-out class"
  ...
  it(`should change the animation of slides when the width of the carousel is between 600 and 900; options 'animateIn' and 'animateOut'`, fakeAsync(() => {
    ...
    expect(oldActiveSlide.classList.contains('owl-animated-out')).toBeTruthy(
      '2th slide has .owl-animated-out class'
    );
    expect(oldActiveSlide.classList.contains('fadeOutDown')).toBeTruthy(
      '2th slide has .fadeOutDown class'
    );

    newActiveSlide = deSlides[2].nativeElement;
    expect(newActiveSlide.classList.contains('animated')).toBeTruthy(
      '3th slide has .animated class'
    );
    expect(newActiveSlide.classList.contains('owl-animated-in')).toBeTruthy(
      '3th slide has .owl-animated-in class'
    );
    expect(newActiveSlide.classList.contains('fadeInDown')).toBeTruthy(
      '3th slide has .fadeInDown class'
    );
    expect(newActiveSlide.classList.contains('active')).toBeTruthy(
      '3th slide has .active class'
    );
    ...
    expect(oldActiveSlide.classList.contains('owl-animated-out')).toBeFalsy(
      "2th slide hasn't .owl-animated-out class"
    );
    ...
    newActiveSlide = deSlides[1].nativeElement;
    expect(newActiveSlide.classList.contains('animated')).toBeFalsy(
      "3th slide hasn't .animated class"
    );
    expect(newActiveSlide.classList.contains('owl-animated-in')).toBeFalsy(
      "3th slide hasn't .owl-animated-in class"
    );
    expect(newActiveSlide.classList.contains('fadeInDown')).toBeFalsy(
      "3th slide hasn't .fadeInDown class"
    );
    ...
    expect(oldActiveSlide.classList.contains('owl-animated-out')).toBeFalsy(
      "3th slide hasn't .owl-animated-out class"
    );
    expect(oldActiveSlide.classList.contains('slideOutDown')).toBeFalsy(
    ...
    expect(newActiveSlide.classList.contains('animated')).toBeFalsy(
      "4th slide hasn't .animated class"
    );
    expect(newActiveSlide.classList.contains('owl-animated-in')).toBeFalsy(
      "4th slide hasn't .owl-animated-in class"
    );
    expect(newActiveSlide.classList.contains('flipInX')).toBeFalsy(
      "4th slide hasn't .flipInX class"
    );
    expect(newActiveSlide.classList.contains('fadeInDown')).toBeFalsy(
      "4th slide hasn't .flipInX class"
    );
    expect(newActiveSlide.classList.contains('active')).toBeTruthy(
      '4th slide has .active class'
    );
  }));
  ...
  it("should set centered slide according to 'fragment' of url after init component with carousel [options]=\"{center: true, URLhashListener:true, startPosition: 'URLHash', nav: true}\"", fakeAsync(() => {
    ...
    let deActiveCenteredSlide: HTMLElement = deCarouselComponent.query(
      By.css('.owl-item.active.center')
    ).nativeElement;
    ...
    deActiveCenteredSlide = deCarouselComponent.query(
      By.css('.owl-item.active.center')
    ).nativeElement;
  ...
  it("shouldn't set centered slide according to 'fragment' of url after init component with carousel [options]=\"{center: true, URLhashListener:true, nav: true}\"", fakeAsync(() => {
    ...
    `;
    TestBed.overrideComponent(HashComponent, {
      set: { template: htmlHashComp },
    });
    fixtureHost = createTestComponent(html);
...
@Component({
  ...
  template: `
    <div style="width: 920px; margin: auto">
      ...
      >
        <ng-template carouselSlide id="owl-slide-1" dataHash="one"
          ><div style="height: 100px">Slide 1</div></ng-template
        >
        <ng-template carouselSlide id="owl-slide-2" dataHash="two"
          ><div style="height: 40px">Slide 2</div></ng-template
        >
        <ng-template carouselSlide id="owl-slide-3" dataHash="three"
          ><div style="height: 80px">Slide 3</div></ng-template
        >
        <ng-template carouselSlide id="owl-slide-4" dataHash="four"
          ><div style="height: 130px">Slide 4</div></ng-template
        >
        <ng-template carouselSlide id="owl-slide-5" dataHash="five"
          ><div style="height: 90px">Slide 5</div></ng-template
        >
      </owl-carousel-o>
    </div>
  `,
